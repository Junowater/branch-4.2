...
function loadData() {
    let savedTeamMembers = localStorage.getItem('frontLine_' + "teamMembers");
    if (savedTeamMembers) {
        try {
    ...
    let savedConstraints = localStorage.getItem('frontLine_' + "constraints");
    if (savedConstraints) {
        try {
    ...
    let savedWorkstations = localStorage.getItem('frontLine_' + "workstations");
    ...
    let savedSchedule = localStorage.getItem('frontLine_' + "schedule");
    if (savedSchedule) {
        try {
            schedule = JSON.parse(savedSchedule);
        } catch (e) {
            console.error("Error parsing saved schedule:", e);
            schedule = {};  // reset schedule on error
        }
    } else {
        schedule = {};  // initialize an empty schedule if none exists
    }

    const savedDisabledQuarters = localStorage.getItem('frontLine_' + "disabledQuarters");
...
function saveData(showAlert = true) {
    localStorage.setItem('frontLine_' + "teamMembers", JSON.stringify(teamMembers));
    localStorage.setItem('frontLine_' + "constraints", JSON.stringify(constraints));
    localStorage.setItem('frontLine_' + "workstations", JSON.stringify(workstations));
    localStorage.setItem('frontLine_' + "schedule", JSON.stringify(schedule));  // <-- Save schedule
    localStorage.setItem('frontLine_' + "disabledQuarters", JSON.stringify(disabledQuarters));
...
function removeTeamMemberFromCell(event, name, quarter, workstation) {
    ...
    generateScheduleTable();
    updateUnassignedBox();
    saveData(false);  // Save changes after removal
}
...
function assignTeamMemberToCell(name, quarter, workstation) {
    ...
    saveData(false);  // Save changes after assignment
}
...
function toggleLockState(event, name, quarter, workstation) {
    ...
    if (assignment) {
        ...
        generateScheduleTable();
        saveData(false);  // Save changes after toggling lock state
    }
}
...
function toggleQuarterAvailability(name, quarter, element) {
    ...
    generateScheduleTable();
    updateUnassignedBox();
    saveData(false);  // Save changes after toggling availability
}
...
function toggleTeamMemberActive(name, element) {
    ...
    updateUnassignedBox();
    generateSkillsTable();
    saveData(false);  // Save changes after toggling active status
}
...
function toggleSkill(name, ws, element) {
    ...
    saveData(false);  // Save changes after toggling skill
}
...
function togglePartialSkill(name, ws, element) {
    ...
    saveData(false);  // Save changes after toggling partial skill
}
...
function deleteTeamMember(name) {
    if (confirm(`Are you sure you want to delete ${name}? This action cannot be undone.`)) {
        ...
        let savedRotations = localStorage.getItem('frontLine_' + "savedRotations");
        if (savedRotations) {
            ...
            localStorage.setItem('frontLine_' + "savedRotations", JSON.stringify(savedRotations));
...
function addTeamMember() {
    ...
    if (allTeamMembers.some(existing => existing.toLowerCase() === name.toLowerCase())) {
    ...
    generateSkillsTable();
    generateTeamMemberPool();
    saveData(false);  // Save changes after adding a new team member
}
...
function drop(ev) {
    ...
    if (sourceQuarter === targetQuarter) {
        ...
        generateScheduleTable();
        updateUnassignedBox();
        saveData(false);  // Save changes after drop within same quarter
        return;
    }
    ...
    generateScheduleTable();
    updateUnassignedBox();
    saveData(false);  // Save changes after drop between quarters
}
...
function dropToPool(ev) {
    ...
    generateScheduleTable();
    updateUnassignedBox();
    saveData(false);  // Save changes after drop to pool
}
...
function rotateAssignments() {
    ...
    if (assignWorkstationsEnhanced(0, teamMemberAssignments, startTime, maxExecutionTime)) {
        ...
        updateUnassignedBox();
        renderSnapshotChart(schedule);
        saveData(false);  // Save changes after successful rotation
    } else {
...
function resetWeeklyData() {
    localStorage.removeItem('frontLine_' + "savedRotations");
...
function rotateAssignmentsPrioritizeNewStation() {
    ...
    if (result) {
        ...
        updateUnassignedBox();
        renderSnapshotChart(schedule);
        saveData(false);  // Save changes after successful prioritized rotation
    } else {
...
function rotateQuarter(quarter) {
    ...
    if (assignOneQuarter(qIdx, teamMemberAssignments, startTime, maxExecutionTime)) {
        ...
        updateUnassignedBox();
        renderSnapshotChart(schedule);
        saveData(false);  // Save changes after rotating a quarter
    } else {
        if (violatedConstraints.size > 0) {
            let audio = new Audio('sounds/alert tone 1.mp3'); // <-- update path if needed
            audio.play().catch(err => {
...
function clearSchedule() {
    ...
    saveData(false);  // Save cleared schedule
}
